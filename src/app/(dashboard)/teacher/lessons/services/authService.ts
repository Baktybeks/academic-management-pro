...
export const authApi = {
  ...
  getCurrentUser: async (): Promise<User | null | { notActivated: true }> => {
    try {
      ...
      // Проверяем, активирован ли пользователь (СуперАдмины автоматически активированы)
  ...
  register: async (
    ...
    try {
      ...
      // Проверяем, есть ли СуперАдмины в системе
      ...
      // Если нет СуперАдминов - первый пользователь будет СуперАдмином
      const finalRole =
        adminCheck.total === 0
  ...
  login: async (email: string, password: string): Promise<User> => {
    try {
      ...
      if (
        ...
        await account.deleteSession("current");
        throw new Error(
          "Ваш аккаунт ожидает активации администратором или академсоветником."
        );
      }
  ...
  // Создание пользователя (для Академсоветников)
  createUser: async (
    name: string,
    email: string,
    password: string,
...
export const useAcademicCouncil = () =>
  useUsersByRole(UserRole.ACADEMIC_COUNCIL);
...
export const useActiveAcademicCouncil = () =>
  useActiveUsersByRole(UserRole.ACADEMIC_COUNCIL);
